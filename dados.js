...
const cardsMarketing = [
  ...
  { key: "visitasPerfil",      label: "Visitas no perfil",   format: "numero",     comparar: true },
  { key: "seguidores",         label: "Seguidores",          format: "numero",     comparar: true },
  ...
  { key: "aprovacoes",         label: "Aprovações",          format: "numero",     percentualKey: "aprovacoesPct", percentualLabel: "dos leads", comparar: true },
...
function encontrarIndicesColunasMarketing(json) {
  ...
  if (json.table && json.table.cols) {
    json.table.cols.forEach((col, idx) => {
      ...
      else if (label.includes("visita")) mapa.visitasPerfil = idx;
      else if (label.includes("seguidor")) mapa.seguidores = idx;
...
function parsearRespostaMarketing(texto) {
  ...
  linhas.forEach((row) => {
    ...
    registros.push({
      ...
      visitasPerfil: idx.visitasPerfil >= 0 ? parsearNumero(row.c[idx.visitasPerfil]) : 0,
      seguidores: idx.seguidores >= 0 ? parsearNumero(row.c[idx.seguidores]) : 0
    });
  });
...
/* Conversões do funil: Aprovações sobre Leads e Vendas sobre Aprovações. */
function calcularIndicadoresMarketing(lancamentos) {
  const ind = {
    investimento: 0, leads: 0, aprovacoes: 0, vendas: 0,
    visitasPerfil: 0, seguidores: 0,
    custoPorLead: 0, aprovacoesPct: 0,
  ...
  lancamentos.forEach((l) => {
    ...
    ind.vendas         += l.vendas;
    ind.visitasPerfil  += l.visitasPerfil;
    ind.seguidores     += l.seguidores;
  });

  ind.custoPorLead     = ind.leads > 0 ? ind.investimento / ind.leads : 0;
  ind.aprovacoesPct    = ind.leads > 0 ? (ind.aprovacoes / ind.leads) * 100 : 0;
...
function parsearRespostaVendedores(texto) {
  ...
  linhas.forEach((row) => {
    ...
    const periodoCell = row.c[0];
    const periodoTexto = periodoCell && (periodoCell.v || periodoCell.f)
      ? String(periodoCell.v || periodoCell.f).trim()
      : "";
    const datasPeriodo = periodoTexto.match(/\d{4}-\d{2}-\d{2}|\d{1,2}[/.]\d{1,2}[/.]\d{2,4}/g) || [];
    const periodoInicioISO = parsearDataIso(datasPeriodo[0] || periodoCell);
    const periodoFimISO = parsearDataIso(datasPeriodo[1] || datasPeriodo[0] || periodoCell);
    const dataISO = periodoInicioISO;
...
function filtrarVendedoresPorIntervalo(dataInicio, dataFim) {
  return todosLancamentosVendedores.filter((l) => {
    const inicioPeriodo = l.periodoInicioISO || l.dataISO;
    const fimPeriodo = l.periodoFimISO || l.dataISO;
    if (dataInicio && fimPeriodo < dataInicio) return false;
    if (dataFim && inicioPeriodo > dataFim) return false;
    return true;
  });
}
